...
/*   Updated: 2026/01/29 01:35:58 by ybutkov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	put_axis_labels(t_app *app)
{
	...
	project_axis(app->map->get_camera(app->map), create_vector(1, 0, 0),
		&end_x, &end_y);
	mlx_string_put(app->mlx, app->win, center_x + end_x + 5, center_y + end_y
		- 5, 0xFF0000, "X");
	project_axis(app->map->get_camera(app->map), create_vector(0, 1, 0),
		&end_x, &end_y);
	mlx_string_put(app->mlx, app->win, center_x + end_x + 5, center_y + end_y
		- 5, 0x00FF00, "Y");
	project_axis(app->map->get_camera(app->map), create_vector(0, 0, 1),
		&end_x, &end_y);
	mlx_string_put(app->mlx, app->win, center_x + end_x + 5, center_y + end_y
		- 5, 0x0000FF, "Z");
}
