...
/*   Updated: 2026/01/29 01:33:26 by ybutkov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_obj	*select_object_at_screen_pos(t_app *app, int screen_x, int screen_y)
{
	...
	ray = get_ray(app->map->get_camera(app->map), ratio.x, ratio.y);
	closest_obj = NULL;
	min_t = INFINITY;
	obj = app->map->objects;
	while (obj)
...
void	move_selected_object(t_app *app, int delta_x, int delta_y)
{
	...
	right = app->map->get_camera(app->map)->right;
	up = app->map->get_camera(app->map)->up;
